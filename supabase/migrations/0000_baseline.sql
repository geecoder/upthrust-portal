-- =============================================================================
-- 0000_baseline.sql — RECONSTRUCTED BASELINE. DO NOT RUN THIS FILE.
-- =============================================================================
-- Upthrust Portal · project qzpuvectpqxmtbitmtlm · captured 2026-09-12
-- Branch: fix/pre-launch-stabilisation
--
-- WHAT THIS IS
-- The starting point of the migration ledger. It records the schema that was
-- ALREADY IN PRODUCTION on 2026-09-12, so that every migration numbered 0001
-- and above has a known base to apply against.
--
-- WHAT THIS IS NOT
-- It is NOT a runnable creation script. Running it against production would
-- fail on every existing object, and running it against an empty database
-- would NOT reproduce production, because the capture method could not read
-- CHECK constraints, UNIQUE constraints, RLS policies, indexes or triggers.
-- Treat it as a record, not as code.
--
-- HOW IT WAS CAPTURED
-- Read-only over PostgREST (HTTP GET/HEAD) with the service-role key. No SQL
-- was executed against the database. PostgREST exposes only public-schema
-- tables and columns, so the following are ABSENT from this baseline and must
-- be filled in by running docs/INTROSPECT.sql and committing the results:
--
--     * CHECK constraint definitions   (blocks a definitive fix for F-16)
--     * UNIQUE constraint definitions
--     * RLS: which tables have it enabled, and every policy body
--     * Indexes
--     * Triggers and functions
--
-- Until those are captured this baseline is INCOMPLETE and is marked as such.
--
-- DRIFT
-- This schema differs from the four root-level .sql files in both directions.
-- See docs/SCHEMA_DRIFT.md. Those four files are historical scripts that were
-- partially applied in unknown order. They are superseded by this ledger and
-- must not be re-run — supabase-weeks-seed.sql in particular would destroy the
-- live curriculum (SCHEMA_DRIFT.md section 6).
--
-- GOING FORWARD
-- Every schema change gets a numbered file here, is reviewed, and is run once
-- in the SQL Editor. No more dashboard edits without a corresponding migration.
-- =============================================================================
