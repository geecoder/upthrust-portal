...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  try {
    ...
    console.error('Portal layout: DB error', err);
  }

  if (!isAdmin && !learner) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--paper-soft)', padding: 24, fontFamily: 'Manrope, sans-serif' }}>
        <div style={{ maxWidth: 480, textAlign: 'center' }}>
          ...
          <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: '1.75rem', fontWeight: 400, marginBottom: 12 }}>Access Pending</h1>
          <p style={{ color: 'var(--ink-muted)', lineHeight: 1.65, marginBottom: 8 }}>
            Your Upthrust portal account is being activated. This usually happens within a few hours of enrollment.
          </p>
          <p style={{ color: 'var(--ink-muted)', lineHeight: 1.65, marginBottom: 24, fontSize: '0.9375rem' }}>
            If it's been more than 24 hours, email{' '}
